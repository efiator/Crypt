package com.ll.crypt;

public class Des
{


    public static int [] permut1(int [] key)
    {
        int [] pc1 ={57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27,19,11,3,60,52,44,36, 63,55,47,39,31,23,15,7,62,54,46,38,30,22,14,6,61,53,45,37,29,21,13,5,28,20,12,4};
        int [] newKey = new int[pc1.length];

        for(int i=0; i<newKey.length;i++)
        {
            newKey[i]=key[pc1[i]-1];

        }
        return newKey;
    }

    public static int [] shift(int [] key)
    {
        int shiftKey[] = new int [key.length];
        int temp = key[0];
        for(int i=0;i<key.length;i++)
        {
            shiftKey[i]=key[i+1];
        }
        shiftKey[key.length-1]=temp;

        return shiftKey;
    }

    public static int [] permut2(int [] key)
    {
        int [] pc2 = {14,17,11,24,1,5,3,28,15,6,21,10,23,19,12,4,26,8,16,7,27,20,13,2,41,52,31,37,47,55,30,40,51,45,33,48,44,49,39,56,34,53,46,42,50,36,29,32};
        int [] newKey = new int[pc2.length];
        for(int i=0; i<newKey.length;i++)
        {
            newKey[i]=key[pc2[i]-1];
        }
        return newKey;
    }

    public static int [] xor(int [] a, int [] b)
    {
        int [] c=new int[a.length];
        for(int i=0; i<c.length;i++)
        {
            if(a[i]==b[i])
            {
                c[i]=0;
            }
            else
            {
                c[i]=1;
            }
        }
        return c;
    }

    public static int [] initPerm(int [] ptxt)
    {
        int [] inPerm=new int [ptxt.length];
        int [] ip={58,50,42,34,26,18,10,2,60,52,44,36,28,20,12,4,62,54,46,38,30,22,14,6,64,56,48,40,32,24,16,8,57,49,41,33,25,17,9,1,59,51,43,35,27,19,11,3,61,53,45,37,29,21,13,5,63,55,47,39,31,23,15,7};
        for(int i=0; i<ptxt.length;i++)
        {
            inPerm[i]=ptxt[ip[i]-1];
        }
        return inPerm;
    }

    public static int [] invPerm(int [] ptxt)
    {
        int [] inPerm=new int [ptxt.length];
        int [] invP = {40,8,48,16,56,24,64,32,39,7,47,15,55,23,63,31,38,6,46,14,54,22,62,30,37,5,45,13,53,21,61,29,36,4,44,12,52,20,60,28,35,3,43,11,51,19,59,27,34,2,42,10,50,50,18,58,26,33,1,41,9,49,17,57,25};
        for(int i=0; i<ptxt.length;i++)
        {
            inPerm[i]=ptxt[invP[i]-1];
        }
        return inPerm;
    }

    public static int [] exPerm(int [] txt)
    {
        int [] newtxt = new int [48];
        int [] e = {32,1,2,3,4,5,4,5,6,7,8,9,8,9,10,11,12,13,12,13,14,15,16,17,16,17,18,19,20,21,20,21,22,23,24,25,24,25,26,27,28,29,28,29,30,31,32,1};
        for(int i=0; i<newtxt.length;i++)
        {
            newtxt[i]=txt[e[i]-1];
        }
        return newtxt;
    }

    public static int [] perm(int [] txt)
    {
        int [] permf = {16,7,20,21,29,12,28,17,1,15,23,26,5,18,31,10,2,8,24,14,32,27,3,9,19,13,30,6,22,11,4,25};
        int [] newtxt = new int [permf.length];
        int j=0;
        for(int i=0; i<newtxt.length;i++)
        {
            newtxt[i]=txt[permf[i]-1];
            j++;
        }
        return newtxt;
    }
}